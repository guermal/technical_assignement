...
def main():
   ...
   "mistral": "mistralai/Mistral-7B-v0.1"
    ...
    # code for initi model and device and optimizer lr ...
    # load a model VIVIT, VIDEOMAE, TIMESFORMER or videoswin
    ...
    # params for dataloader fixed for sake of dummy code
    ...
    train_transforms = {
      'spatial':  Compose([MultiScaleRandomCropMultigrid(scales, crop_size),#RandomHorizontalFlip(),
                           ToTensor(255),
    ...
    vid_seq_train = base_loader(config['gt_folder'],config['seq_len'],config['num_classes'],config['captioning'],train_transforms['spatial'],train_transforms['temporal'],mode='train')

    
    train_loader = torch.utils.data.DataLoader(vid_seq_train, batch_size=config['batch_size'],shuffle=True, num_workers=4, pin_memory=True)
    ...
    vid_seq_test = base_loader(config['gt_folder'],config['seq_len'],config['num_classes'],config['captioning'],validation_transforms['spatial'],validation_transforms['temporal'],mode='test')
    test_loader = torch.utils.data.DataLoader(vid_seq_test, batch_size=1, shuffle=False, num_workers=4,
                                                pin_memory=True)

    train(epochs,model,train_loader,test_loader,optimizer,scheduler,criterion,model_name,device,config['captioning'],config['use_vlm'])

    #test_only(epochs,model, train_loader, test_loader,optimizer, scheduler, criterion,device)
    
    #train(epochs,model, train_loader, test_loader,optimizer, scheduler, criterion,device,config['model'],task=config['task'])
